...
export default function StepProgressBar() {
  ...
  const goTo = (href: string) => () => router.push(`#${href}`, { scroll: false });

  return (
    <div className="xl:flex hidden fixed right-40 h-[100%] justify-center items-center">
      <div className="h-[65%] w-[100%] flex flex-col items-center justify-center absolute px-24">
      ...
      <div className="h-[70%] w-[100%] flex flex-col items-center justify-between absolute px-24">
        {sections.map((s, i) =>
          <div className="w-16 h-16 text-slate-400 flex justify-center items-center z-20">
            {<s.icon
              onClick={goTo(s.title)}
              size={50}
              className="cursor-pointer"
              style={{
                color: getCurrentSectionIndex(scrollPercentage, sections.length) === i ? "white" : undefined,
